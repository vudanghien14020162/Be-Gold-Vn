...
const DEFAULT_TABLE     = "log_crawl_pnj";
const COMPANY_PNJ       = 3;
exports.insertLogsPNJ = async function (datas) {
    try {
        if(datas && datas.length){
            const values = datas
                .map(r =>
                    `(${sequelize.escape(r.name)}, ${r.buy_raw}, ${r.sell_raw}, ${r.buy}, ${r.sell}, ${sequelize.escape(r.date)}, '${r.source}', ${sequelize.escape(r.last_update)}, ${sequelize.escape(r.area)})`
                )
                .join(",");

            const sql = `
            INSERT INTO log_crawl_pnj
        ...
        console.log("EX insertLogsPNJ", e);
        return false;
    }
};
...
exports.getLastUpdateTime = async () => {
    try {
        const result = await sequelize.query(
            "SELECT last_update FROM `log_crawl_pnj` order by id DESC limit 1", {
