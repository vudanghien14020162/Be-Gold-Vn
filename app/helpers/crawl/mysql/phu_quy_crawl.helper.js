...
const phuquy_gold_helper = require("./phu_quy_gold.helper"); // giống btmc_gold_helper / doji_gold_helper
...
exports.fetchGiavangOrgPhuQuy = async function fetchGiavangOrgPhuQuy() {
    ...
    rows.each((index, row) => {
        ...
        // Nếu tên vàng rỗng → dùng loại vàng
        ...
        if(buy < 3000000 && name.trim() == ""){
            // Nếu tên vàng rỗng → dùng loại vàng
            nameGold = 'Bạc thỏi Phú Quý 999 (1Kilo, 10 lượng, 5 lượng, 1 lượng)';
        }
        items.push({
            name: nameGold,
            area: area || "Toàn quốc",
            ...
            buy,
            sell,
            // Giờ Việt Nam rõ ràng, không cộng 7 giờ linh tinh nữa
