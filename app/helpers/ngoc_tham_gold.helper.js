...
const DEFAULT_TABLE     = "log_crawl_ngoc_tham";
const COMPANY_NGOC_THAM = 8;

exports.insertLogsNgocTham = async function (datas) {
    try {
        if(datas && datas.length){
            const values = datas
                .map(r =>
                    `(${sequelize.escape(r.name)}, ${sequelize.escape(r.buy_raw)}, ${sequelize.escape(r.sell_raw)}, ${r.buy}, ${r.sell}, ${sequelize.escape(r.date)}, '${r.source}', ${sequelize.escape(r.last_update)}, ${sequelize.escape(r.area)})`
                )
                .join(",");

            const sql = `
            INSERT INTO log_crawl_ngoc_tham
                (name, buy_raw, sell_raw, buy, sell, date, source, last_update, area)
            VALUES ${values};
        `;
        ...
        console.log("EX insertLogsNgocTham", e);
        return false;
    }
};
...
exports.getLastUpdateTime = async () => {
    try {
        const result = await sequelize.query(
            "SELECT last_update FROM `log_crawl_ngoc_tham` order by id DESC limit 1", {
        ...
        console.log("Ex getLastUpdateTimeNgocTham",e);
...
exports.insertCrawledPricesWithDiffYesterday = async (items, table = DEFAULT_TABLE) => {
    ...
    try {
        ...
        for (const item of items) {
            ...
            const company_id = COMPANY_NGOC_THAM;
